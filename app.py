...
from models import health_check_agents
from database import health_check_database, init_database
...
def startup_health_checks() -> bool:
    ...
    if config.langsmith_enabled:
        ...
        try:
            ...
            # Actual connection test happens during first API call
    ...
    # ====================================================================
    if not config.skip_health_checks and config.is_development:
        logger.info("6. Running end-to-end smoke test...")
        try:
            # TODO: Add smoke test with dummy image
            logger.info("   ⚠ Smoke test skipped (not implemented)")
            health_results["Smoke Test"] = (True, "Skipped")
        except Exception as e:
            logger.warning(f"   ⚠ Smoke test failed: {e}")
            health_results["Smoke Test"] = (False, f"Failed: {e}")
            # Don't fail startup on smoke test failure
...
def main():
    ...
    # This avoids runtime conflicts with Streamlit's own server
    cmd = [
        sys.executable,
        "-m",
        "streamlit",
